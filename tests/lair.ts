  ...
  describe('#createRecords', () => {
    ...
    describe('create sequences', () => {
      it('getNextValue should receive a list with previous values', () => {
        ...
        const A = Factory.create({
          attrs: {
            propA: Factory.sequenceItem<number | string>(
              'initial',
              (prevValues) => {
      ...
      it('getNextValue should receive only part of the previous values', () => {
        ...
        const A = Factory.create({
          attrs: {
            propA: Factory.sequenceItem<number>(
              1,
              (v) => {
      ...
      it('getNextValue (not arrow) should be called in the new record context', () => {
        ...
        const A = Factory.create({
          attrs: {
            propA: Factory.sequenceItem<string | number>(
              1,
    ...
    describe('common', () => {
      ...
      describe('#createOne', () => {
        ...
        it('should create record with custom id if `allowCustomIds` is `true`', () => {
          lair.registerFactory(
            Factory.create({ attrs: {}, allowCustomIds: true }),
            'baz'
          );
          expect(lair.createOne('baz', { id: 'custom_id' })).to.have.property(
            'id',
            'custom_id'
          );
        });
        ...
        it('should throw an error if value for field is not exist in the `allowedValues`', () => {
          ...
          expect(() => lair.createOne('baz', { a: 4 })).to.throw(
      ...
      describe('#updateOne', () => {
        ...
        it('should throw an error if value for field is not exist in the `allowedValues`', () => {
          ...
          expect(() => lair.updateOne('baz', '1', { a: 4 })).to.throw(
            `"a" must be one of the "1,2,3". You passed "4"`
          );
        });
    ...
    describe('with related records', () => {
      ...
      describe('RU methods should allow to ignore some related data', () => {
        ...
        describe('#createOne', () => {
          ...
          it('get `a` and ignore related `b`', () => {
            expect(lair.createOne('a', {}, { ignoreRelated: ['b'] })).to.be.eql(
              { id: '2' }
            );
          });

          it('get `b` and ignore related `a`', () => {
            expect(lair.createOne('b', {}, { ignoreRelated: ['a'] })).to.be.eql(
              { id: '2', propC: [] }
            );
          });

          it('get `c` and ignore related `b`', () => {
            expect(lair.createOne('c', {}, { ignoreRelated: ['b'] })).to.be.eql(
              { id: '2' }
            );
          });

          it('get `a` and ignore all related', () => {
            expect(lair.createOne('a', {}, { ignoreRelated: true })).to.be.eql({
              id: '2',
            });
          });

          it('get `a` and not ignore anything related', () => {
            expect(lair.createOne('a', {}, { ignoreRelated: false })).to.be.eql(
              { id: '2', propB: [] }
            );
          });
        });
